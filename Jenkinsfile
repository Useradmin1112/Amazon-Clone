...
pipeline  {
    ...
    stages {
        ...
        stage("Change IP in axios.js")
         {
             steps{
                sh "find FrontEnd/my-app/ -type f -exec sed  -i 's#http://localhost:5034#http://20.14.80.247:5034#g' {} +"
             }
         }
         stage("Change IP in appsettings.json")
         {
             steps{
                sh "find BackEnd/Amazon-clone/ -type f -exec sed  -i 's#http://localhost:81#https://20.14.80.247/#g' {} +"
             }
         }
        stage("Change Database IP in appsettings.json")
         {
             steps{
                sh "find BackEnd/Amazon-clone/ -type f -exec sed  -i 's#Server=20.240.61.200#Server=20.14.80.247#g' {} +"
             }
         }
 ...
 '''
        ...
        stage("docker backend push") {
            steps {
                ...
                sh '''
                docker push Useradmin1112/amazon-clone-backend
                '''
            }
        }
    }
